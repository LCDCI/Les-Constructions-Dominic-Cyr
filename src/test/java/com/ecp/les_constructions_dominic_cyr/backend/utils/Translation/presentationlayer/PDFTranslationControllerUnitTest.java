...
class PDFTranslationControllerUnitTest {
    ...
    @Test
    void translatePdf_WithValidPdfFile_ReturnsOk() {
        ...
        when(deepLService.translatePdf(any(byte[].class), anyString(), anyString()))
                .thenReturn(Mono.just("translated pdf".getBytes()));
        when(cacheService.generateCacheFilename(anyString(), anyString())).thenReturn("test_fr.pdf");
        when(cacheService.saveTranslation(anyString(), anyString(), any(byte[].class))).thenReturn(true);
        ...
        verify(deepLService, times(1)).translatePdf(any(byte[].class), anyString(), anyString());
        verify(cacheService, times(1)).saveTranslation(anyString(), anyString(), any(byte[].class));
    }
    ...
    @Test
    void translatePdf_WithSourceLanguage_TranslatesToOpposite() {
        DataBuffer dataBuffer = DefaultDataBufferFactory.sharedInstance.wrap(testPdfData);
        ...
        when(deepLService.translatePdf(any(byte[].class), eq("EN"), eq("FR")))
                .thenReturn(Mono.just("translated pdf".getBytes()));
        when(cacheService.generateCacheFilename(anyString(), anyString())).thenReturn("test_fr.pdf");
        when(cacheService.saveTranslation(anyString(), anyString(), any(byte[].class))).thenReturn(true);
        ...
        verify(deepLService, times(1)).translatePdf(any(byte[].class), eq("EN"), eq("FR"));
    }
    @Test
    void translatePdf_WithTargetLanguage_UsesTargetLanguage() {
        DataBuffer dataBuffer = DefaultDataBufferFactory.sharedInstance.wrap(testPdfData);
        ...
        when(cacheService.getCachedTranslation(anyString(), anyString())).thenReturn(null);
        when(deepLService.toDeepLLanguageCode("en")).thenReturn("EN");
        when(deepLService.translatePdf(any(byte[].class), anyString(), eq("EN")))
                .thenReturn(Mono.just("translated pdf".getBytes()));
        when(cacheService.generateCacheFilename(anyString(), anyString())).thenReturn("test_en.pdf");
        when(cacheService.saveTranslation(anyString(), anyString(), any(byte[].class))).thenReturn(true);
        
        StepVerifier.create(controller.translatePdf(Mono.just(filePart), null, "en"))
                .assertNext(response -> {
                    assertEquals(HttpStatus.OK, response.getStatusCode());
                })
                .verifyComplete();
        
        verify(deepLService, times(1)).translatePdf(any(byte[].class), anyString(), eq("EN"));
    }
    ...
    @Test
    void translatePdf_WithNullFilename_UsesDefault() {
        DataBuffer dataBuffer = DefaultDataBufferFactory.sharedInstance.wrap(testPdfData);
        ...
        verify(cacheService, times(1)).getCachedTranslation("document.pdf", "fr");
    }

    @Test
    void translatePdf_WithPdfContentType_AcceptsFile() {
        DataBuffer dataBuffer = DefaultDataBufferFactory.sharedInstance.wrap(testPdfData);
        HttpHeaders headers = new HttpHeaders();
