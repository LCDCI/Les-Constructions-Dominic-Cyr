...
CREATE INDEX IF NOT EXISTS idx_footer_section ON footer_content(section);
CREATE INDEX IF NOT EXISTS idx_footer_display_order ON footer_content(display_order);
...
CREATE INDEX IF NOT EXISTS idx_project_identifier ON projects(project_identifier);
...
CREATE TABLE IF NOT EXISTS lots (
                      ...
                      project_id VARCHAR(255) NOT NULL, -- Changed to VARCHAR to match data.sql
                      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- Reference project_identifier (the string) instead of project_id (the number)
...
-- schema for project_overview_content
CREATE TABLE IF NOT EXISTS project_overview_content (
...
-- schema for project_gallery_images
...
CREATE INDEX IF NOT EXISTS idx_project_features_identifier ON project_features(project_identifier);
CREATE INDEX IF NOT EXISTS idx_project_gallery_identifier ON project_gallery_images(project_identifier);

-- schema for renovations
...
CREATE TABLE IF NOT EXISTS project_management_page_content (
                                                 id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_pm_content_language ON project_management_page_content(language);

-- schema for app_theme
...
CREATE INDEX IF NOT EXISTS idx_analytics_reports_owner ON analytics_reports(owner_id);
CREATE INDEX IF NOT EXISTS idx_analytics_reports_timestamp ON analytics_reports(generation_timestamp DESC);
