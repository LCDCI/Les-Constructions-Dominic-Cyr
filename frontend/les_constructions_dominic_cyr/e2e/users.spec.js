...
test.describe('Users Page - Display', () => {
  ...
  test('should show loading state initially', async ({ page }) => {
    ...
    const isLoading = await usersPage.loadingMessage.isVisible().catch(() => false);
    const isTableVisible = await usersPage.usersTable.isVisible().catch(() => false);
    const isNoUsers = await usersPage.noUsersMessage.isVisible().catch(() => false);
  ...
  test('should display users table or no users message after loading', async () => {
    ...
    const hasNoUsersMessage = await usersPage.noUsersMessage.isVisible().catch(() => false);
...
test.describe('Users Page - Add User Modal', () => {
  ...
  test('should allow filling all form fields', async () => {
    ...
    expect(await usersPage.primaryEmailInput.inputValue()).toBe('john.doe@example.com');
    expect(await usersPage.secondaryEmailInput.inputValue()).toBe('john.secondary@example.com');
...
test.describe('Users Page - Edit User Modal', () => {
  ...
  test('should allow editing first name and last name', async () => {
    ...
    if (userCount > 0) {
      ...
      expect(await usersPage.editFirstNameInput.inputValue()).toBe('UpdatedFirst');
      expect(await usersPage.editLastNameInput.inputValue()).toBe('UpdatedLast');
    }
  });
});
...
test.describe('Users Page - Error Handling', () => {
  ...
  test('should display error modal on create user failure', async ({ page }) => {
    ...
    const errorModal = page.locator('.modal-backdrop:has-text("User Operation Failed")');
