...
async function loginAsOwner(page) {
  ...
  if (!currentUrl.includes('localhost:3000') || currentUrl.includes('login')) {
    ...
    throw new Error(
      `Login failed. Expected to be at localhost:3000/, but at: ${currentUrl}. Check debug-login-failed.png`
    );
  }
  ...
  test.beforeEach(async ({ page }) => {
    ...
    if (currentUrl.includes('login')) {
      await page.screenshot({
        path: 'debug-users-redirect.png',
        fullPage: true,
      });
      throw new Error(
        `Cannot access /users - redirected to login. URL: ${currentUrl}. Check debug-users-redirect.png`
      );
    }
