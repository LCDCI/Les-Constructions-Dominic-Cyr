...
test.describe('Edit Project Form - Positive Tests', () => {
  ...
  test('should populate form with existing project data', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const originalName = await projectsPage.projectTitles
        .first()
        .textContent();

      await projectsPage.clickEditProject(0);
      await expect(
        projectsPage.page.locator('.edit-project-form')
      ).toBeVisible();
  ...
  test('should update project name successfully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const inputValue = await projectsPage.page
        .locator('input#projectName')
        .inputValue();
  ...
  test('should update project description successfully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      // Verify the value was entered
      const inputValue = await projectsPage.page
        .locator('textarea#projectDescription')
        .inputValue();
  ...
  test('should update project location successfully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const locationInput = projectsPage.page
        .locator('input#location, div[class*="autocomplete"] input')
        .first();
  ...
  test('should update progress percentage successfully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      // Verify the value was entered
      const inputValue = await projectsPage.page
        .locator('input#progressPercentage')
        .inputValue();
  ...
  test('should change project status successfully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const selectedValue = await projectsPage.page
        .locator('select#status')
        .inputValue();
  ...
  test('should cancel edit without saving', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      // Make a change
      const originalInput = await projectsPage.page
        .locator('input#projectName')
        .inputValue();
  ...
  test('should update multiple fields at once', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const nameValue = await projectsPage.page
        .locator('input#projectName')
        .inputValue();
      const descValue = await projectsPage.page
        .locator('textarea#projectDescription')
        .inputValue();
      const locInput = projectsPage.page
        .locator('input#location, div[class*="autocomplete"] input')
        .first();
...
test.describe('Edit Project Form - Negative Tests', () => {
  ...
  test('should display error when project name is empty', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const errorMessages = await projectsPage.page
  ...
  test('should display error when location exceeds max length', async ({
    page,
  }) => {
    const projectCount = await projectsPage.getProjectCount();
    ...
    if (projectCount > 0) {
      ...
      expect(errorMessages.length).toBeGreaterThan(0);
      expect(
        errorMessages.some(msg => msg.includes('exceed') || msg.includes('255'))
      ).toBeTruthy();
    }
  });
  ...
  test('should display error when end date is before start date', async ({
    ...
    if (projectCount > 0) {
      ...
      expect(
        errorMessages.some(
          msg => msg.includes('End date') || msg.includes('after')
        )
      ).toBeTruthy();
    }
  });

  test('should not display edit button if user lacks permissions', async ({
    page,
  }) => {
  ...
  test('should handle form submission errors gracefully', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const formVisible = await projectsPage
        .isEditFormVisible()
        .catch(() => false);
      const errorVisible = await projectsPage.page
        .locator('.error-message')
        .isVisible()
        .catch(() => false);
...
test.describe('Edit Project Form - Form Interactions', () => {
  ...
  test('should clear field errors when user corrects input', async ({
    page,
  }) => {
    const projectCount = await projectsPage.getProjectCount();
  ...
  test('should display form sections correctly', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const basicInfoSection = projectsPage.page.locator(
        'text=Basic Information'
      );
      const statusDatesSection = projectsPage.page.locator(
        'text=Status & Dates'
      );
  ...
  test('should show required field indicator', async ({ page }) => {
    ...
    if (projectCount > 0) {
      ...
      const requiredIndicators = await projectsPage.page
        .locator('.required')
        .count();
