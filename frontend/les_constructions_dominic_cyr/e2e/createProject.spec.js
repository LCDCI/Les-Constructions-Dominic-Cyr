...
test.describe('Create New Project', () => {
  ...
  test('S1 — Success: Create project with full flow (step 0 → 1 → 2, no lots, no cover); assert UI + data tier', async ({
    ...
    await page.waitForURL(/\/(projects\/[^/]+|projects)(\/)?$/, { timeout: 15000 }).catch(() => {});

    // --- Data tier: Project must exist in API ---
    const listRes = await page.request.get('/api/v1/projects');
    expect(listRes.ok()).toBeTruthy();
    const projects = await listRes.json();
    const names = Array.isArray(projects) ? projects.map(p => p.projectName || p.name) : [];
    expect(
  ...
  test('S2 — Success variant: New project with no lots; assert UI + data tier', async ({
    ...
    const projects = await listRes.json();
    const names = Array.isArray(projects) ? projects.map(p => p.projectName || p.name) : [];
    expect(names.some(n => (n || '').includes(data.projectName))).toBeTruthy();
  });
  ...
  test('A1 — Alternate: Cancel from step 0 (French); form closes, no project in data tier', async ({
    ...
    const countBefore = await page.request.get('/api/v1/projects').then(r => r.ok() ? r.json() : []).then(p => (Array.isArray(p) ? p.length : 0)).catch(() => 0);

    await createPage.clickCancel();
    await createPage.waitForFormToClose();

    const countAfter = await page.request.get('/api/v1/projects').then(r => r.ok() ? r.json() : []).then(p => (Array.isArray(p) ? p.length : 0)).catch(() => 0);
  ...
  test('A4 — Alternate: Back from step 1 to step 0; still on form, no project created', async ({
    ...
    const countAfter = await page.request.get('/api/v1/projects').then(r => r.ok() ? r.json() : []).then(p => (Array.isArray(p) ? p.length : 0)).catch(() => 0);
    const names = await page.request.get('/api/v1/projects').then(r => r.ok() ? r.json() : []).then(p => (Array.isArray(p) ? p.map(x => x.projectName || x.name) : [])).catch(() => []);
  ...
  test('V1 — Validation: Missing required fields at step 0; cannot proceed, no project created', async ({
    ...
    const isDisabled = await fillOutBtn.getAttribute('disabled').then(a => a != null);
    if (!isDisabled) {
      await fillOutBtn.click();
      await page.waitForTimeout(500);
      const errorText = await createPage.getValidationOrErrorText();
      expect(errorText || await createPage.errorMessages.count() > 0).toBeTruthy();
    }
  ...
  test('V2 — Validation: Try to continue to lots without filling English required fields; blocked or error', async ({
    ...
    const isDisabled = await continueBtn.getAttribute('disabled').then(a => a != null);
  ...
  test('V3 — Validation: End date before start date; validation message, no project created', async ({
    ...
    const hasDateError =
      errorText &&
      (errorText.includes('date') || errorText.includes('End date') || errorText.includes('after') || errorText.includes("avant"));
    expect(hasDateError || (await createPage.errorMessages.count()) > 0).toBeTruthy();
  });
  });
  ...
  test('U1 — Form title in English when app language is English', async ({ page }) => {
  ...
  test('U2 — Form title in French when app language is French', async ({ page }) => {
    const translationPage = new TranslationPage(page);
    await translationPage.goto('/');
    await translationPage.waitForTranslationsToLoad();
    const currentLang = await translationPage.getCurrentLanguage();
  ...
  test('E1 — API error: Create project request fails; error shown, form remains or closes with message', async ({
    ...
    const errorVisible =
      ...
      (await page.locator('text=/failed|error|unavailable/i').isVisible().catch(() => false));
  ...
  test('Create project via API and verify it appears in UI list (UI ↔ data tier)', async ({
    ...
    expect(createResponse.status(), 'createProject should return 201').toBe(201);
