...
const ReportList = ({ refreshTrigger }) => {
  ...
  const handleDownload = async (reportId, format) => {
    try {
      ...
      link.setAttribute(
        'download',
        `Audit_Report_${new Date().getTime()}.${extension}`
      );
      ...
      alert(
        'Failed to download the file. Please check if the file service is running.'
      );
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      alert(
        'Error: The server could not delete this report. This usually happens if the file service is unreachable.'
      );
      fetchReports();
    } finally {
      setDeleting(null);
  ...
  return (
    <div className="report-list">
          ...
          <tbody>
            {reports.map(report => (
              <tr key={report.id}>
                <td>
                  <span
                    className={`format-badge ${report.fileFormat.toLowerCase()}`}
                  >
                    {report.fileFormat}
                  </span>
                </td>
                ...
                <td>
                  {new Date(report.generationTimestamp).toLocaleDateString()}
                </td>
      ...
      <DeleteConfirmationModal
        ...
        reportName={
          reportToDelete
            ? `Audit - ${new Date(reportToDelete.generationTimestamp).toLocaleDateString()}`
            : ''
        }
      />
    </div>
