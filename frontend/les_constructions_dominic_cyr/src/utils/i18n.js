  ...
  try {
    ...
    for (const lang of languagesToLoad) {
      ...
      if (allTranslations && Object.keys(allTranslations).length > 0) {
        console.log(`[i18n] Loaded ${lang} translations with namespaces:`, Object.keys(allTranslations));
        ...
        for (const [namespace, translations] of Object.entries(allTranslations)) {
          if (translations && typeof translations === 'object') {
            ...
            if (Object.keys(globalTranslations).length > 0) {
              i18n.addResourceBundle(lang, 'translation', globalTranslations, true, true);
              console.log(`[i18n] Added nav/footer to 'translation' namespace for ${lang} (from ${namespace})`);
            }
          }
        }
        ...
        if (allTranslations.messages && typeof allTranslations.messages === 'object') {
          i18n.addResourceBundle(lang, 'translation', allTranslations.messages, true, true);
          console.log(`[i18n] Added 'messages' content to 'translation' namespace for ${lang} (backward compatibility)`);
        }
...
i18n.changeLanguage = async (lng, callback) => {
  ...
  if (!hasResources || Object.keys(i18n.getResourceBundle(lng, 'translation') || {}).length === 0) {
