    ...
    if (status && status !== 'ALL') {
      if (status === 'APPROVED') {
        result = result.filter(q =>
          ['OWNER_APPROVED', 'CUSTOMER_APPROVED'].includes(q.status)
        );
      } else {
  ...
  const handleApprovalConfirm = async rejectionReason => {
    ...
    try {
      if (approvalAction === 'APPROVE') {
        ...
        showNotification(
          'success',
          t('quote.approval.approveSuccess') || 'Quote approved successfully'
        );
      } else {
        await quoteApi.rejectQuote(
        ...
        showNotification(
          'success',
          t('quote.approval.rejectSuccess') || 'Quote rejected successfully'
        );
      }
  ...
  return (
    <div className="quote-approval-page">
      <div className="page-header">
        ...
        <p>
          {t('quote.approval.subtitle') ||
            'Review and manage contractor quotes'}
        </p>
      </div>
      ...
      <div className="filters-section">
        <div className="filter-group">
          <label>
            <MdSearch /> {t('common.search') || 'Search'}
          </label>
          <input
            type="text"
            className="search-input"
            placeholder={
              t('quote.approval.searchPlaceholder') ||
              'Quote #, Project, Contractor...'
            }
            value={searchTerm}
        ...
        <div className="filter-group">
          <label>
            <MdFilterList /> {t('quote.status') || 'Status'}
          </label>
          <select
          ...
          >
            ...
            <option value="SUBMITTED">
              {t('quote.approval.submitted') || 'Pending Review'}
            </option>
            <option value="APPROVED">
              {t('quote.approval.approved') || 'Approved'}
            </option>
            <option value="REJECTED">
              {t('quote.approval.rejected') || 'Rejected'}
            </option>
          </select>
        </div>
      ...
      <div className="quotes-table-container">
        {filteredQuotes.length === 0 ? (
          <div className="empty-state">
            <p>
              {t('quote.noQuotes') || 'No quotes found matching your filters.'}
            </p>
          </div>
        ) : (
              ...
              {filteredQuotes.map(quote => (
                <tr key={quote.quoteNumber}>
                  <td
                    ...
                    data-label={t('quote.number') || 'Quote #'}
                  >
                    {quote.quoteNumber}
                  </td>
                  ...
                  <td
                    ...
                    data-label={t('quote.contractor') || 'Contractor'}
                  >
                    {contractorNames[quote.contractorId] || quote.contractorId}
                  </td>
                  <td
                    className="quote-date"
                    data-label={t('quote.date') || 'Date'}
                  >
                    {formatDate(quote.createdAt)}
                  </td>
                  <td
                    className="quote-amount"
                    data-label={t('quote.amount') || 'Amount'}
                  >
                    {formatCurrency(quote.totalAmount)}
                  </td>
                  <td
                    className="quote-status"
                    data-label={t('quote.status') || 'Status'}
                  >
                    <span
