...
export default function ProjectPhotosPage() {
  ...
  const doDelete = async () => {
    try {
      ...
      if (!actor) {
        setActionError(
          'Cannot determine your user identifier. Please reload and try again.'
        );
        return;
      }
      console.debug('[ProjectPhotosPage] Deleting file', {
  ...
  const doArchive = async () => {
    try {
      ...
      const msg =
        err.response?.data?.error || err.message || 'Failed to archive photo.';
      setActionError(msg);
    }
  };
    ...
    <div className="photos-page">
        ...
        <div className="photos-actions">
          ...
          >
            ...
            <span style={{ marginLeft: 6 }}>
              {isRefreshing ? 'Syncing...' : 'Sync Photos'}
            </span>
          </button>
        ...
        <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border:
                filter === 'active' ? '1px solid #6366F1' : '1px solid #E5E7EB',
              background: filter === 'active' ? '#EEF2FF' : '#fff',
            }}
          >
            Active
          </button>
            ...
            style={{
              ...
              border:
                filter === 'archived'
                  ? '1px solid #6366F1'
                  : '1px solid #E5E7EB',
              background: filter === 'archived' ? '#EEF2FF' : '#fff',
            }}
          >
            Archived
          </button>
        ...
        {normalizeFiles(allFiles)
          ...
          .map(file => (
            <PhotoCard
              ...
              canUnarchive={
                (canUnarchive || filter === 'archived') && file.isArchived
              }
            />
          ))}
      </div>
      ...
      <ConfirmationModal
        ...
        config={{
          ...
          cancelText: 'Cancel',
          isDestructive:
            modalState.action === 'delete' || modalState.action === 'archive',
        }}
      />
