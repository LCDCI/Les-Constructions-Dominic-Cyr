...
export default function ProjectTeamManagementPage() {
  ...
  const loadProjectData = async () => {
    try {
      ...
      try {
        const activityLogsData = await projectApi.getProjectActivityLog(
          projectId,
          token
        );
        ...
        console.warn(
          'Unable to load activity logs, continuing without them:',
          activityErr
        );
        setActivityLog([]);
      }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const contractorsChanged =
        JSON.stringify([...selectedContractorIds].sort()) !==
        JSON.stringify([...previousContractorIds].sort());
      const salespersonsChanged =
        JSON.stringify([...selectedSalespersonIds].sort()) !==
        JSON.stringify([...previousSalespersonIds].sort());
      ...
      const inactiveCustomerToAdd =
        selectedCustomerId && selectedCustomerId !== previousCustomerId
      ...
      if (
        inactiveCustomerToAdd &&
        inactiveCustomerToAdd.userStatus !== 'ACTIVE'
      ) {
        const confirmed = await new Promise(resolve => {
          setUserToReactivate({
      ...
      if (salespersonsChanged) {
        ...
        for (const salespersonId of selectedSalespersonIds) {
          await projectApi.assignSalespersonToProject(
            projectId,
            salespersonId,
            token
          );
      ...
      if (customerChanged) {
        if (previousCustomerId && !selectedCustomerId) {
          ...
          // Assign new customer
          await projectApi.assignCustomerToProject(
            projectId,
            selectedCustomerId,
            token
          );
  ...
  const getUserStatusLabel = userStatus => {
    ...
    return (
      userStatus.charAt(0).toUpperCase() + userStatus.slice(1).toLowerCase()
    );
  };
  ...
  const getStatusBadge = userStatus => {
    ...
    const colors = {
      ...
      Deactivated: {
        bg: 'var(--status-deactivated-bg)',
        color: 'var(--status-deactivated-color)',
        border: 'var(--status-deactivated-border)',
      },
      Unknown: {
        bg: 'var(--status-inactive-bg)',
        color: 'var(--status-inactive-color)',
  ...
  const filteredContractors = contractors.filter(c => {
    ...
    if (statusFilter === 'inactive')
      return c.userStatus === 'INACTIVE' || c.userStatus === 'DEACTIVATED';
    return true;
  });
  ...
  const filteredSalespersons = salespersons.filter(s => {
    ...
    if (statusFilter === 'inactive')
      return s.userStatus === 'INACTIVE' || s.userStatus === 'DEACTIVATED';
    return true;
  });
          ...
          <section className="info-section">
            ...
            <ul className="project-info-list">
              ...
              <li>
                ...
                {project.buyerName || 'Not specified'}
              </li>
            </ul>
          </section>
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    backgroundColor:
                      statusFilter === 'all' ? 'var(--primary-color)' : 'white',
                    color:
                      statusFilter === 'all' ? 'white' : 'var(--primary-color)',
                    cursor: 'pointer',
                    fontSize: '13px',
                    fontWeight: '500',
                  }}
              ...
              <div className="contractors-list">
                {filteredSalespersons.map(salesperson => (
                  <div
                    ...
                    className="contractor-checkbox"
                  >
                    <input
                      type="checkbox"
                      id={`sp-${salesperson.userIdentifier}`}
                      checked={selectedSalespersonIds.includes(
                        salesperson.userIdentifier
                      )}
                      onChange={e => {
                        if (e.target.checked) {
                          ...
                          setSelectedSalespersonIds(
                            selectedSalespersonIds.filter(
                              id => id !== salesperson.userIdentifier
                            )
                          );
                        }
                      }}
                    ...
                    />
                    <label
                      htmlFor={`sp-${salesperson.userIdentifier}`}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                ...
                {filteredContractors.map(contractor => (
                  ...
                  >
                    <input
                      ...
                      checked={selectedContractorIds.includes(
                        contractor.userIdentifier
                      )}
                      onChange={e => {
                        if (e.target.checked) {
                          ...
                          setSelectedContractorIds(
                            selectedContractorIds.filter(
                              id => id !== contractor.userIdentifier
                            )
                          );
                        }
                      }}
                    ...
                    />
                    <label
                      htmlFor={contractor.userIdentifier}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                  ...
                  {filteredCustomers.map(customer => (
                    ...
                    >
                      ...
                      {customer.userStatus !== 'ACTIVE'
                        ? `(${getUserStatusLabel(customer.userStatus)})`
                        : ''}
                    </option>
                  ))}
                </select>
      ...
      {showReactivateConfirm && userToReactivate && (
        ...
        >
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <p>
              The selected {userToReactivate.role}{' '}
              <strong>
                {userToReactivate.user.firstName}{' '}
                {userToReactivate.user.lastName}
              </strong>{' '}
              is currently{' '}
              {getUserStatusLabel(
                userToReactivate.user.userStatus
              ).toLowerCase()}
              .
            </p>
            <p>
              Do you want to reactivate this user and assign them to the
              project?
            </p>
